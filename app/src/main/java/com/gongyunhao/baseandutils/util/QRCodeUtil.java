...
//  來源：简书
//
//  Creste by GongYunHao on 2018/3/28
// 
public class QRCodeUtil {
    ...
    public Bitmap generateBitmap(String content, int width, int height) {
        ...
        try {
            /**
             * encode返回一个BitMatrix，相当于是一个二维数组
             * BitMatrix中的get方法的返回值为一个boolean类型，true表示该点有数据，false表示该点没有数据。
            ...
            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    if (encode.get(j, i)) {                //如果矩阵上该像素点有数据，就设置成黑色
            ...
            /**
             * 调用Bitmap的createBitmap方法创建一个Bitmap出来
